...
public class RezopiaServiceTestes1 extends BaseConfigurations {
	...
	private Object executeGetService(String serviceName, int statusCode) {
		...
		try {
			return given().contentType(CONTENT_TYPE).log().everything()
					.expect().statusCode(statusCode).log().everything().when()
					.get(service.getUrl()).asString();
		} catch (Exception e) {
			if (e instanceof UnknownHostException) {
				System.out.println("Service Not Avaliable");
				Assert.assertEquals("404", statusCode, "Service Not Avaliable");
	...
	private Object executePostService(String serviceName, int statusCode) {
		...
		try {
			return given().contentType(CONTENT_TYPE).body(service.getRequest())
					.log().everything().expect().statusCode(statusCode).log()
					.everything().when().post(service.getUrl()).asString();
		} catch (Exception e) {
			if (e instanceof UnknownHostException) {
				System.out.println("Service Not Avaliable");
				Assert.assertEquals("404", statusCode, "Service Not Avaliable");
	...
	public static void setup() {
		// new PrintStream(new FileOutputStream("example.txt", true))
