...
public class CustomizedEmailableReport implements IReporter {
	...
	private static final Logger L = Logger
			.getLogger(CustomizedEmailableReport.class);
	...
	private int getPassCount = 0;
	private int getFailedCount = 0;
	private int getSkippedCount = 0;
	// ~ Methods --------------------------------------------------------------
	...
	public void loadProperties() {
		...
		try {
			...
			projectProperties.load(fileInput);
	...
	public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites,
		...
		getPassedAndFailedCounts(suites);
		startHtml(out);
		logoHtml(out);
		headerHTML(out, "TestSuites Summary ");
		generateSuiteSummaryReport(suites);
		headerHTML(out, "Test Cases Details Summary");
		generateMethodSummaryReport(suites);
		headerHTML(out, "Test Cases Complete Details ");
		out.println("</center>");
	...
	protected PrintWriter createWriter(String outdir) throws IOException {
		 ...
		 * if(eReport.exists()){ eReport.delete();
		 * System.out.println("Deleted"); }
		 */
		return new PrintWriter(new BufferedWriter(new FileWriter(new File(
				outdir, "customized-emailable-report-"+new SimpleDateFormat("dd-MM-yyyy'.html'").format(new Date())))));
	...
	protected void generateMethodDetailReport(List<ISuite> suites) {
		...
		for (ISuite suite : suites) {
			...
			/*	if (r.values().size() > 0) {
				...
				resultDetail(testContext.getFailedConfigurations(),"fail");
				resultDetail(testContext.getFailedTests(),"fail");
				...
				if(style.equalsIgnoreCase("passed"))
				{
					buff.append("<td>"
							...
							+ (null == testInstanceName ? "" : "<br>("
									+ testInstanceName + ")")
							+ "</td>"
							+ "<td class=\"numi\" style=\"text-align:left;padding-right:2em;width:30%\">"
							+ firstLine + "<br/>" + screenshotLnk + "</td>"
	...
	private void startResultSummaryTable(String style) {
		...
		out.println("<tr bgcolor='#DA70D6'><th>TestClass</th>"
				+ "<th>Method</th><th style=\"width:10%\"'>Exception</th><th>Start Time </th><th>Time<br/>(hh:mm:ss)</th></tr>");
		row = 0;
	}
	...
	private void resultDetail(IResultMap tests,String resultStatus) {
		...
		for (ITestResult result : list) {
			...
			methodIndex++;
			if(resultStatus.equalsIgnoreCase("skipped") || resultStatus.equalsIgnoreCase("fail"))
			{
	...
	private Collection<ITestNGMethod> getMethodSet(IResultMap tests,
		...
		List<ITestNGMethod> allMethods = new ArrayList<ITestNGMethod>(
	...
	public void generateSuiteSummaryReport(List<ISuite> suites) {
		...
		out.print("<tr bgcolor='#DA70D6'>");
		tableColumnStart("TestSuite");
	...
	//	tableColumnStart("Excluded<br/>Groups");
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r : tests.values()) {
				...
				SimpleDateFormat summaryFormat = new SimpleDateFormat(
						"kk:mm:ss");
				 ...
				 */
				summaryCell(
						millisToTimeConversion((overview.getEndDate().getTime() - overview
		...
		if (qty_tests > 1) {
			 ...
			 */
			summaryCell(
					millisToTimeConversion(((time_end - time_start) / 1000)),
					true);
	...
	private void startSummaryRow(String label) {
		...
		out.print("<tr"
				...
				+ testIndex + "\" title='Click for More Details' ><b>" + label + "</b></a>" + "</td>");
	...
	protected void startHtml(PrintWriter out) {
		...
		out.println(".failedodd td,.numi_attn {background-color: #FF3C33}");
		out.println(".failedeven td,.stripe .numi_attn {background-color: #FF3C33}");
		out.println(".stacktrace {white-space:pre;font-family:monospace}");
		out.println(".totop {font-size:85%;text-align:center;border-bottom:2px solid #000}");
		out.println("</style>");
		out.println("<script type='text/javascript' src='https://www.google.com/jsapi'></script>");
		String piediagram = "function drawChart () {"
		...
		out.println("<script type='text/javascript'>" + piediagram
				+ "</script>");
		out.println("</head>");
		out.println("<body style='border-style: double;margin-right: 70px;margin-bottom: 10px;margin-left: 70px;margin:margin: 100px 150px 100px 80px;'>");
	}

	protected void logoHtml(PrintWriter out) {
		out.println("<center><br/><br/><img src='"+getPropertyValue("company_logo")+"' width='90' height='60'></img><h1>"+getPropertyValue("company_title")+"</h1><h2>"+getPropertyValue("report_title")+"--"+
				new SimpleDateFormat("dd/MM/yyyy").format(new Date())
				+ "</h2>");
		out.println("<hr style='height:5;width:90%;border-top: solid blue;'></hr>");
		out.println("<div id='chart_div'></div>");
	}

	protected void headerHTML(PrintWriter out, String message) {
		out.println("<h3 style='color:blue;'>" + message + "</h3>");
	}

	/** Finishes HTML stream */
	protected void endHtml(PrintWriter out) {
		out.println("<center><b>RestAutomate @ CopyRights</b> </center>");
	...
	private class TestSorter implements Comparator<IInvokedMethod> {
		...
		public int compare(IInvokedMethod o1, IInvokedMethod o2) {
			...
			int r = o1.getTestMethod().getTestClass().getName()
					.compareTo(o2.getTestMethod().getTestClass().getName());
	...
	private void getPassedAndFailedCounts(List<ISuite> suites) {
		if (suites != null) {
			for (ISuite suite : suites) {
				Map<String, ISuiteResult> tests = suite.getResults();
				for (ISuiteResult r : tests.values()) {
					ITestContext overview = r.getTestContext();
					int q = getMethodSet(overview.getFailedTests(), suite)
							.size();
					getFailedCount = getFailedCount + q;
					q = getMethodSet(overview.getPassedTests(), suite).size();
					getPassCount = getPassCount + q;
					q = getMethodSet(overview.getSkippedTests(), suite).size();
					getSkippedCount = getSkippedCount + q;
